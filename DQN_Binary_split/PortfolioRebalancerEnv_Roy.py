...
class PortfolioRebalancerEnv(gym.Env):
    ...
    def __init__(self,
        ...
        # cumulative tc cost
        self.tc_cum_cost = {key: [] for key in self.action_space}
    ...
    def step(self, action_delta):
        ...
        if not terminated:
            ...
            # reward = -reward
            cost_i = cost_turnover(w0=self.state, w1=new_state, tc=self.transaction_costs) * self.scaling_factor
            # self.tc_cum_cost[action_delta[0]].append(cost_i)
            sharpe = net_sharpe(w1=new_state, mu=self.mu, cov=self.cov)
            reward = sharpe - cost_i  # np.sum(self.tc_cum_cost[action_delta[0]])
